...
16. **Per-route rate limits must be declared *inline* in the route options — never via an `onRoute` hook.** `@fastify/rate-limit` is registered globally in `server.ts` (100/min, `skipOnError`) as the runtime safety net, but CodeQL's `js/missing-rate-limiting` only credits a per-route limit when it's a literal `{ config: { rateLimit: {...} } }` passed in the route's *options* object — its `FastifyPerRouteRateLimit` model matches `app.<method>(path, opts, handler)` (a ≥3-arg setup whose `opts` carries `config.rateLimit`). A limit injected by mutating `route.config` inside an `onRoute` hook *enforces at runtime but is invisible to static analysis*, so the route keeps alerting "performs authorization, but is not rate-limited" — which is exactly what happened to `POST /api/submit` until the limit was inlined. The **global** registration is likewise unseen by CodeQL: the model only matches the legacy *unscoped* `fastify-rate-limit` import, not the scoped `@fastify/rate-limit` this repo uses. Net: inline per-route `config.rateLimit` is the only shape that clears these alerts. Tiers in use: brute-force/abuse surfaces strict (`/api/submit` 5/min; `/admin/login`, `/admin/account/password`, `/claim/:token` 10/min), admin mail-sending APIs 30/min (`/admin/api/invite-requests/:id/{approve,deny}`), admin reads 60/min (`/admin/api/invitations`, `/admin/players`); every route without an inline limit falls back to the global 100/min. If you tighten a route, add the inline option there — don't reach for a hook. Tests in `app/test/submit.test.ts`.
